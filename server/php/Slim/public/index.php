    ...
    if (!empty($user)) {
        if($user->is_email_confirmed != 1) {
            if (md5($user['username']) == $request->getAttribute('hash')) {
                ...
                echo '<script>location.replace("'.$_server_domain_url.'?success=0");</script>';exit;
            } else {
                return renderWithJson($result, 'Invalid user details.', '', 1);
            }
        } else {
...
$app->GET('/api/v1/contestants/highest_votes_list', function ($request, $response, $args) {
    ...
    try {
		...
		if (!empty($contests)) {
			...
			$highest_votes = Models\User::with($enabledIncludes)->where('is_email_confirmed', true)->where('is_active', true)->where('role_id', \Constants\ConstUserTypes::Employer)->where('votes','<>', 0)->where('votes','>', 0)->limit(10)->orderBy('votes', 'DESC')->get()->toArray();
			$highest_votes_list = array();
			if ((!empty($highest_votes))) {
				$highest_votes_list['data'] = (!empty($highest_votes)) ? $highest_votes : array();
			} else {
...
$app->GET('/api/v1/admin/payment_gateways', function ($request, $response, $args) {
    ...
    try {
    	$count = 1000;
		$queryParams['is_active'] = true;
		$respones = Models\PaymentGateway::with('attachment')->Filter($queryParams)->paginate($count);
		$respones = $respones->toArray();
        $data = $respones['data'];
        unset($respones['data']);
        $result = array(
...
$app->GET('/api/v1/admin/judges', function ($request, $response, $args) {
    ...
    try {
		...
		$queryParams['is_active'] = true;
        $respones = Models\Judges::Filter($queryParams)->paginate($count);
		$respones = $respones->toArray();
        $data = $respones['data'];
        unset($respones['data']);
        $result = array(
...
$app->POST('/api/v1/admin/votes', function ($request, $response, $args) {
	...
	try {
		...
		if (!empty($contestant_details)) {
			$contestant_details->votes = $contestant_details->votes + $args['votes'];
...
$app->DELETE('/api/v1/admin/advertisement/{id}', function ($request, $response, $args) {
    global $authUser;
	$args = array();
		...
		if (!empty($vote_package)) {
			...
			if (!empty($paymentGateway)) {
				try {
					...
					$hash = encrypt_decrypt('encrypt', $userDetailId.'/'.$queryParams['username'].'/'.$queryParams['category_id'].'/'.$queryParams['payment_gateway_id'].'/'.$is_sanbox.'/'.$request->getAttribute('packageId').'/'.$isGuest);
					if ($paymentGateway['name'] == 'PayPal') {
		...
		if (!empty($pay_data)) {
			...
			if (!empty($response) && $response['ack'] == 'success' && !empty($response['response'])) {
				if (strtolower($response['response']['status']) == 'completed') {
					...
					foreach($response['response']['paymentInfoList']['paymentInfo'] as $paymentInfo) {
						if ($i == 0) {
							insertTransaction(0, 1, \Constants\TransactionClass::VotePackage, \Constants\TransactionType::VotePackage, $payment_gateway_id, $paymentInfo['receiver']['amount'], 0, 0, 0, 0, $foreign_id, $is_sanbox, $user_id, $paymentInfo['transactionStatus'], $paymentInfo['transactionId'], $paymentInfo['senderTransactionId'], $isGuest);
						}
						if ($i == 1) {
							insertTransaction(0, $contestantInfo->company_id, \Constants\TransactionClass::VotePackage, \Constants\TransactionType::VotePackage, $payment_gateway_id, $paymentInfo['receiver']['amount'], 0, 0, 0, 0, $foreign_id, $is_sanbox, $user_id, $paymentInfo['transactionStatus'], $paymentInfo['transactionId'], $paymentInfo['senderTransactionId'], $isGuest);
						}
						if ($i == 2) {
							insertTransaction(0, $contestant_id, \Constants\TransactionClass::VotePackage, \Constants\TransactionType::VotePackage, $payment_gateway_id, $paymentInfo['receiver']['amount'], 0, 0, 0, 0, $foreign_id, $is_sanbox, $user_id, $paymentInfo['transactionStatus'], $paymentInfo['transactionId'], $paymentInfo['senderTransactionId'], $isGuest);
						}
						$i++;
					}
					insertTransaction($user_id, 0, \Constants\TransactionClass::VotePackage, \Constants\TransactionType::VotePackage, $payment_gateway_id, $vote_package->price, 0, 0, 0, 0, $foreign_id, $is_sanbox, $contestant_id ,$response['response']['paymentInfoList']['paymentInfo'][0]['transactionStatus'], $response['response']['paymentInfoList']['paymentInfo'][0]['transactionId'], $response['response']['paymentInfoList']['paymentInfo'][0]['senderTransactionId'], $isGuest);
					if ($isGuest != 1) {
						$user->is_paid = true;
						$user->save();
					} else {
